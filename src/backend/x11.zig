...
const X11Backend = struct {
    ...
    fn drawBorder(app: *X11Backend, width: i32, height: i32) void {
        if (app.gc) |gc| {
            ...
            var corner_y: i32 = 0;
            while (corner_y < config.window.corner_radius) : (corner_y += 1) {
                const dx: i32 = @intFromFloat(@sqrt(@as(f64, @floatFromInt(config.window.corner_radius * config.window.corner_radius - corner_y * corner_y))));
                _ = x11.XDrawPoint(app.display, app.window, gc, config.window.corner_radius - dx, corner_y);
                _ = x11.XDrawPoint(app.display, app.window, gc, width - config.window.corner_radius + dx, corner_y);
                _ = x11.XDrawPoint(app.display, app.window, gc, config.window.corner_radius - dx, height - 1 - corner_y);
                _ = x11.XDrawPoint(app.display, app.window, gc, width - config.window.corner_radius + dx, height - 1 - corner_y);
            }

            var corner_x: i32 = 0;
            while (corner_x < config.window.corner_radius) : (corner_x += 1) {
                const dy: i32 = @intFromFloat(@sqrt(@as(f64, @floatFromInt(config.window.corner_radius * config.window.corner_radius - corner_x * corner_x))));
                _ = x11.XDrawPoint(app.display, app.window, gc, corner_x, config.window.corner_radius - dy);
                _ = x11.XDrawPoint(app.display, app.window, gc, width - 1 - corner_x, config.window.corner_radius - dy);
                _ = x11.XDrawPoint(app.display, app.window, gc, corner_x, height - 1 - config.window.corner_radius + dy);
                _ = x11.XDrawPoint(app.display, app.window, gc, width - 1 - corner_x, height - 1 - config.window.corner_radius + dy);
            }
        }
    }

    fn drawApplications(app: *X11Backend, ui_state: *ui.State, width: i32, height: i32) void {
        const list_start_y: i32 = config.ui.input_height + 20;
        ...
        _ = x11.XftDrawString8(app.draw, &text_color, app.font, 20, @divTrunc(config.ui.input_height, 2), &ui_state.text_buffer[0], @intCast(ui_state.text_len));

        if (app.gc) |gc| {
            _ = x11.XSetForeground(app.display, gc, x11.XBlackPixel(app.display, x11.XDefaultScreen(app.display)));
            _ = x11.XDrawLine(app.display, app.window, gc, 10, config.ui.input_height, width - 10, config.ui.input_height);
        }
        ...
        while (i < display_count) : (i += 1) {
            ...
            const y: i32 = list_start_y + @as(i32, @intCast(i)) * config.ui.item_height + @divTrunc(config.ui.item_height, 2) + 5;

            var item_text_color: x11.XftColor = undefined;

            if (filtered_idx == ui_state.selected_index) {
                if (app.gc) |gc| {
                    var sel_color = x11.XColor{ .red = config.colors.selection_red, .green = config.colors.selection_green, .blue = config.colors.selection_blue, .flags = 7, .pixel = 0, .pad = 0 };
                    _ = x11.XAllocColor(app.display, app.colormap, &sel_color);
                    _ = x11.XSetForeground(app.display, gc, sel_color.pixel);
                    _ = x11.XFillRectangle(app.display, app.window, gc, 10, list_start_y + @as(i32, @intCast(i)) * config.ui.item_height + 2, @intCast(width - 20), @intCast(config.ui.item_height - 4));
                }
                _ = x11.XftColorAllocValue(app.display, app.visual, app.colormap, &x11.XRenderColor{ .red = 65535, .green = 65535, .blue = 65535, .alpha = 65535 }, &item_text_color);
            } else {
                _ = x11.XftColorAllocValue(app.display, app.visual, app.colormap, &x11.XRenderColor{ .red = 0, .green = 0, .blue = 0, .alpha = 65535 }, &item_text_color);
            }

            _ = x11.XftDrawString8(app.draw, &item_text_color, app.font, 20, y, app_entry.name.ptr, @intCast(app_entry.name.len));
        }
    ...
    fn runX11(backend_ctx: *X11Backend, allocator: std.mem.Allocator) !void {
        ...
        const height_proximity_factor: c_int = 100;
        ...
        var y: i32 = @divTrunc(screen_height - height - 400 - height_proximity_factor, 2);
        ...
        const black = x11.XBlackPixel(backend_ctx.display, screen_num);
        const white = x11.XWhitePixel(backend_ctx.display, screen_num);

        var window_attrs: x11.XSetWindowAttributes = undefined;
        window_attrs.background_pixel = white;
        window_attrs.border_pixel = black;
        ...
        _ = x11.XStoreName(backend_ctx.display, backend_ctx.window, "Launcher");
        ...
        var py: i32 = 0;
        while (py < config.window.corner_radius) : (py += 1) {
            const dx: i32 = @intFromFloat(@sqrt(@as(f64, @floatFromInt(config.window.corner_radius * config.window.corner_radius - py * py))));
            _ = x11.XDrawLine(backend_ctx.display, mask_pixmap, mask_gc, 0, py, config.window.corner_radius - dx, py);
            _ = x11.XDrawLine(backend_ctx.display, mask_pixmap, mask_gc, width - config.window.corner_radius + dx, py, width, py);
            _ = x11.XDrawLine(backend_ctx.display, mask_pixmap, mask_gc, 0, height - 1 - py, config.window.corner_radius - dx, height - 1 - py);
            _ = x11.XDrawLine(backend_ctx.display, mask_pixmap, mask_gc, width - config.window.corner_radius + dx, height - 1 - py, width, height - 1 - py);
        }

        var px: i32 = 0;
        while (px < config.window.corner_radius) : (px += 1) {
            const dy: i32 = @intFromFloat(@sqrt(@as(f64, @floatFromInt(config.window.corner_radius * config.window.corner_radius - px * px))));
            _ = x11.XDrawLine(backend_ctx.display, mask_pixmap, mask_gc, px, 0, px, config.window.corner_radius - dy);
            _ = x11.XDrawLine(backend_ctx.display, mask_pixmap, mask_gc, width - 1 - px, 0, width - 1 - px, config.window.corner_radius - dy);
            _ = x11.XDrawLine(backend_ctx.display, mask_pixmap, mask_gc, px, height - 1 - config.window.corner_radius + dy, px, height - 1);
            _ = x11.XDrawLine(backend_ctx.display, mask_pixmap, mask_gc, width - 1 - px, height - 1 - config.window.corner_radius + dy, width - 1 - px, height - 1);
        }

        _ = x11.XShapeCombineMask(backend_ctx.display, backend_ctx.window, x11.ShapeBounding, 0, 0, mask_pixmap, x11.ShapeSet);

        _ = x11.XFreeGC(backend_ctx.display, mask_gc);
        _ = x11.XFreePixmap(backend_ctx.display, mask_pixmap);

        _ = x11.XMapRaised(backend_ctx.display, backend_ctx.window);
        _ = x11.XSetInputFocus(backend_ctx.display, backend_ctx.window, x11.RevertToPointerRoot, x11.CurrentTime);
        _ = x11.XFlush(backend_ctx.display);
        ...
        if (backend_ctx.draw) |d| {
            x11.XftDrawDestroy(d);
        }
        if (backend_ctx.font) |f| {
            x11.XftFontClose(backend_ctx.display, f);
        }
        if (backend_ctx.gc) |gc| {
            _ = x11.XFreeGC(backend_ctx.display, gc);
        }
